...
class HTTPProvider(BaseProvider):
    ...
    def _validate(self):
        ...
        if self.provides and self.resource_type not in ('photo', 'video', 'rich', 'link'):
            raise ValueError(
                'resource_type must be one of "photo", "video", "rich" or "link"')
    ...
    def convert_to_resource(self, headers, raw_response, params):
        ...
        if content_type in ('text/javascript', 'application/json'):
            ...
            raise OEmbedException(
                'Invalid mime-type - %s' % headers['content-type'])
        return resource
    ...
    def request_resource(self, url, **kwargs):
        ...
        if '?' in self.endpoint_url:
            url_with_qs = '%s&%s' % (
                self.endpoint_url.rstrip('&'), urlencode(params))
        else:
...
class DjangoProviderOptions(object):
    ...
    def _validate(self):
        if self.model is None:
            raise OEmbedException(
                'Provider %s requires a model' % self.provider_class.__name__)
        if not self.named_view:
            raise OEmbedException(
                'Provider %s requires a named_view' % self.provider_class.__name__)
...
class DjangoProvider(BaseProvider):
    ...
    def _path_regex(self):
        ...
        try:
            ...
            raise OEmbedException(
                'Error looking up %s' % self._meta.named_view)
        regex.append(pattern)
        regex = ''.join(regex)
        return regex
    ...
    def get_object(self, url):
        ...
        for key, value in self._meta.fields_to_match.iteritems():
            try:
                ...
                raise OEmbedException(
                    '%s was not found in the urlpattern parameters.  Valid names are: %s' % (key, ', '.join(params.keys())))
    ...
    def resize(self, image_field, new_width=None, new_height=None):
        ...
        if isinstance(image_field, ImageFieldFile) and \
           ...
           image_field.field.height_field:
            # use model fields
            current_width = getattr(
                image_field.instance, image_field.field.width_field)
            current_height = getattr(
                image_field.instance, image_field.field.height_field)
        else:
            # use PIL
            try:
    ...
    def map_to_dictionary(self, url, obj, **kwargs):
        ...
        if 'thumbnail_url' in mapping:
            mapping['thumbnail_url'] = relative_to_full(
                mapping['thumbnail_url'], url)
...
class DjangoDateBasedProvider(DjangoProvider):
    ...
    def _validate(self):
        ...
        if not self._meta.date_field:
            raise OEmbedException(
                'date_field not found for %s' % self.__class__.__name__)
